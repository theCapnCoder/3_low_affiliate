import Slide1 from "../../../public/images/slider/slide_1.jpg";
import Slide2 from "../../../public/images/slider/slide_2.jpg";
import Slide3 from "../../../public/images/slider/slide_3.jpg";
import SlideLarge from "../../../public/images/slider/slide_large.jpg";

export const LatestCards = [
  {
    id: 1,
    img: Slide1,
    alt: "slide",
    text: "Secrets to Know About Hedge Trimming",
  },
  {
    id: 2,
    img: Slide2,
    alt: "slide",
    text: "What to Do With a Dying Tree",
  },
  {
    id: 3,
    img: Slide3,
    alt: "slide",
    text: "Which Flowers Should You Plant Garden",
  },
];

export const LatestFooterCards = [
  {
    id: 1,
    img: SlideLarge,
    alt: "slide",
    text: "How to Prep Your Lawn in the Spring",
  },
  {
    id: 2,
    img: Slide1,
    alt: "slide",
    text: "Secrets to Know About Hedge Trimming",
  },
  {
    id: 3,
    img: Slide2,
    alt: "slide",
    text: "What to Do With a Dying Tree",
  },
];
